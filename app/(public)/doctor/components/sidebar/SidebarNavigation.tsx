...
const SidebarNavigation: React.FC = () => {
  ...
  return (
    <nav
      className="flex flex-col items-start px-0 pt-2 w-60 bg-white gap-[542px] pb-[514px] max-md:gap-96 max-md:px-0 max-md:pt-1.5 max-md:pb-96 max-sm:gap-52 max-sm:px-0 max-sm:pt-1 max-sm:pb-52"
      ...
      <div className="flex flex-col items-start w-60">
        <ul className="w-full">
          ...
          >
            <SubNavItem
              label="환자 명단"
              onClick={() => router.push("/doctor/patient/list")}
            />
            <SubNavItem
              label="표준 치료 프로그램"
          ...
          <NavItem
            icon={<MdOutlinePeople className="nav-icon text-2xl"  />}
            label="의사관리"
            isActive={false}
            onClick={() => router.push("/doctor/master")}
          />

          <NavSection icon={myPageIcon} label="마이페이지">
            <SubNavItem
              label="개인정보 수정"
              onClick={() => router.push("/doctor/mypage/check")}
            />
          </NavSection>
        </ul>
      </div>
