        ...
        return (
          <div 
            ...
            className={`flex justify-center items-center self-stretch px-2.5 py-7 my-auto min-h-[68px] ${column.flex} ${
              column.key === 'status' ? 'font-bold leading-none text-center whitespace-nowrap' : 'whitespace-nowrap'
            }`}
          >
            {column.key === 'status' ? (
              renderCellContent(column.key, cellValue)
            ) : (
              <div className={`truncate ${column.key === 'patientName' || column.key === 'doctor' ? 'text-ellipsis' : ''}`}>
                {renderCellContent(column.key, cellValue)}
              </div>
            )}
          </div>
